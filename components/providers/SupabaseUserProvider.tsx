...
export const SupabaseUserProvider: FC<SupabaseUserProviderProps> = ({
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (_isMounted) {
        // Only update if the session has actually changed (compare access tokens)
        // This prevents unnecessary re-renders on window focus when Supabase broadcasts the same session
        ...
        setUser((prevUser) => {
          if (
            prevUser?.id === newSession?.user?.id &&
            prevUser?.updated_at === newSession?.user?.updated_at
          ) {
            return prevUser;
          }
