 ...
 */
const SUPPORT_EMAIL = config.resend.supportEmail || 'support@ridesharetahoe.com';
  ...
  try {
    ...
    if (!parsed.success) {
      ...
      return {
        ...
        errors: {
          ...flattened.errors,
          _: flattened.errors.length > 0 ? flattened.errors : undefined,
        },
      };
    }
      ...
      const emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="margin-top: 20px; padding: 15px; background: #e3f2fd; border-radius: 8px; font-size: 14px; color: #666;">
            ...
            <p>This message was sent from the RideShareTahoe contact form.</p>
          </div>
        </div>
      `;
...
This message was sent from the RideShareTahoe contact form.
      `;
      ...
      return {
        ...
        errors: {
          _: [
            `Failed to send message. Please try again or contact support directly at ${SUPPORT_EMAIL}.`,
          ],
        },
      };
    }
