...
export interface EmailQueueItem {
  ...
  sendFunction: (emailData: EmailData) => Promise<unknown>;
  options: {
    priority: 'high' | 'normal';
    retries: number;
    maxRetries: number;
...
export class EmailQueue {
   ...
   * @param sendFunction - Function to send the email
   * @param options - Queue options
   * @returns The unique queue item ID
   */
  async addToQueue(
    emailData: EmailData,
    // eslint-disable-next-line no-unused-vars
   ...
   * @returns Result of sendFunction
   */
  async sendEmail(queueItem: EmailQueueItem): Promise<unknown> {
   ...
   */
  async handleRetry(queueItem: EmailQueueItem) {
   ...
   */
  updateRateLimit(config: Partial<EmailRateLimitConfig>): void {
    this.rateLimit = {
      ...this.rateLimit,
      ...config,
    };
